...
def plot_images(
    ...
    if np.max(depths[0]) <= 1:
        depths *= 255  # de-normalise (optional)

    # Build Image
    mosaic = np.full((int(ns * h), int(ns * w), 3), 255, dtype=np.uint8)  # init
    mosaic_depth = np.full((int(ns * h), int(ns * w), 3), 255, dtype=np.uint8)  # init
    for i in range(bs):
        x, y = int(w * (i // ns)), int(h * (i % ns))  # block origin
        mosaic[y : y + h, x : x + w, :] = images[i].transpose(1, 2, 0)
        mosaic_depth[y : y + h, x : x + w, :] = depths[i].transpose(1, 2, 0)
    ...
    if scale < 1:
        ...
        mosaic_depth = cv2.resize(mosaic_depth, tuple(int(x * ns) for x in (w, h)))

    # Annotate
    fs = int((h + w) * ns * 0.01)  # font size
    annotator = Annotator(mosaic, line_width=round(fs / 10), font_size=fs, pil=True, example=names)
    annotator_depth = Annotator(mosaic_depth, line_width=round(fs / 10), font_size=fs, pil=True, example=names)
    for i in range(bs):
        x, y = int(w * (i // ns)), int(h * (i % ns))  # block origin
        annotator.rectangle([x, y, x + w, y + h], None, (255, 255, 255), width=2)  # borders
        annotator_depth.rectangle([x, y, x + w, y + h], None, (255, 255, 255), width=2)  # borders
        if paths:
            annotator.text((x + 5, y + 5), text=Path(paths[i]).name[:40], txt_color=(220, 220, 220))  # filenames
            annotator_depth.text((x + 5, y + 5), text=Path(paths[i]).name[:40], txt_color=(220, 220, 220))  # filenames
        if len(cls) > 0:
            idx = batch_idx == i
            ...
            if len(bboxes):
                ...
                for j, box in enumerate(boxes.astype(np.int64).tolist()):
                    ...
                    if labels or conf[j] > conf_thres:
                        ...
                        annotator_depth.box_label(box, label, color=color, rotated=is_obb)
                ...
                for c in classes:
                    ...
                    annotator_depth.text((x, y), f"{c}", txt_color=color, box_style=True)
                    annotator_depth.text((x, y), f"{c}", txt_color=color, box_style=True)
            ...
            if len(kpts):
                ...
                for j in range(len(kpts_)):
                    if labels or conf[j] > conf_thres:
                        ...
                        annotator_depth.kpts(kpts_[j], conf_thres=conf_thres)
            ...
            if len(masks):
                ...
                im_depth = np.asarray(annotator_depth.im).copy()
                for j in range(len(image_masks)):
                    if labels or conf[j] > conf_thres:
                        ...
                        with contextlib.suppress(Exception):
                            ...
                            im_depth[y: y + h, x: x + w, :][mask] = (
                                    im_depth[y: y + h, x: x + w, :][mask] * 0.4 + np.array(color) * 0.6
                            )
                annotator.fromarray(im)
                annotator_depth.fromarray(im_depth)
    if not save:
        return np.asarray(annotator.im), np.asarray(annotator_depth.im)
    annotator.im.save(fname)  # save
    annotator_depth.im.save(fname_depth)  # save
    if on_plot:
        on_plot(fname)
        on_plot(fname_depth)
