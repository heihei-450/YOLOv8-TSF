...
from ultralytics.nn.yolo12 import (
    A2C2f,
    AAttn,ABlock)
...
class BaseModel(nn.Module):
    ...
    def forward(self, x, *args, **kwargs):
        ...
        Args:
            x (list[torch.Tensor] | dict): The input image tensor or a dict including image tensor and gt labels.
    ...
    def _predict_once(self, x, x2, profile=False, visualize=False, embed=None):
        ...
        for m in self.model:
            ...
            if profile:
                self._profile_one_layer(m, x, x2, dt)
            if m.f == -4:
                x = m(x2)
            else:
                x = m(x)  # run
    ...
    def _profile_one_layer(self, m, x,x2, dt):
        """
        ...
        flops = thop.profile(m, inputs=[x.copy(),x2.copy()] if c else [x,x2], verbose=False)[0] / 1e9 * 2 if thop else 0  # FLOPs
        t = time_sync()
        for _ in range(10):
            if c:
                m(x.copy(),x2.copy())
...
class RTDETRDetectionModel(DetectionModel):
    ...
    def predict(self, x, x2, profile=False, visualize=False, batch=None, augment=False, embed=None):
        """
...
def parse_model(d, ch, verbose=True):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        if m in {
            ...
            C3x,
            RepC3,
            VSSBlock_YOLO, XSSBlock,SimpleStem, VisionClueMerge,
            C2PSA,C3k2,
            A2C2f,AAttn,ABlock,CSPStage,SPPF_attention,ContextGuidedBlock_Down,ContextAggregation,
            C2PSA_Biformer,C2PSA_DLKA,C2PSA_BSAM,CARAFE,BiFPN_Add2,C2f_ParallelPolarized,
        }:
            if f == -4:
                c1, c2 = 3, args[0]
            else:
                if m in [BiFPN_Add2, BiFPN_Add3]:
                    # 多输入模块：f是列表（如[-2, -3]），取所有输入通道数的最大值作为c1
                    c1 = max([ch[x] for x in f])  # 统一输入通道数为最大值
                else:
                    # 单输入模块：f是整数，直接取对应通道数
                    c1 = ch[f]
                c2 = args[0]  # 输出通道数仍从参数获取
